define([
	"text!templates/dsp/report/realtime.html"
	, "text!templates/dsp/report/realtime-table.html"
	, "util/compareChart"
	...
	return Backbone.View.extend({
		...
		render:function(){
			var self = this;
			app.leftNav.show("report","realtime");
		...
		renderState:function(){
			...
			query.start =today; 
			// console.log("dspId: ", appdspId);
			var bidderId = app.getBidder();
			app.Util.get("/dsp/"+ bidderId +"/report/realtime",query,function(err,todayData){
				if(!err){
					query.start = self.choseDateStr;
					console.log('bidderId: ', bidderId);
					app.Util.get("/dsp/"+ bidderId +"/report/realtime",query,function(err,cData){
						var nowHH = moment().format("HH")+1; 
						if(!err){
							self.todayData = _.clone(todayData.reverse());
							self.cData = _.clone(cData.reverse());
		...
		renderChart:function(todayData,cData){
			...
			this.compareChart.setOption($("#compareChart"),{data:[_.clone(todayData),_.clone(cData)],enabled:[type]});
